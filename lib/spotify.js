...
export async function searchArtists(query, limit = 5) {
  ...
  if (!res.ok) throw new Error(`Spotify artist search error ${res.status}`)
  const data = await res.json()
...
export async function analyzeArtistProfile(spotifyUrl) {
  ...
  const [artist, topTracksResult] = await Promise.all([
    getArtist(artistId),
    getArtistTopTracks(artistId).catch(() => []),
  ])
  const topTracks = topTracksResult ?? []

  // Spotify returns empty genres for many mainstream artists.
  // Fall back to related artists' genres, which are almost always populated.
  let genres = artist.genres ?? []
  if (genres.length === 0) {
    const related = await getRelatedArtists(artistId).catch(() => [])
    genres = related.flatMap(a => a.genres ?? [])
  }

  const trackIds = topTracks.slice(0, 10).map(t => t.id)

  // Audio features are restricted for apps created after Nov 2023 — treat as optional
  let features = []
  try {
    features = await getBulkAudioFeatures(trackIds)
  } catch (err) {
    console.warn('[Spotify] Audio features unavailable (likely API restriction):', err.message)
  }
  ...
  const detectedGenre = mapToSoundPulseGenre(genres)
  const detectedVibes = avg ? deriveVibes(avg, genres) : deriveVibesFromGenres(genres)
  ...
  return {
    ...
    // Confident if we detected a genre (audio features are a bonus, not required)
    confident: detectedGenre !== null,
  }
}
...
// Note: this regex approach is a fast fallback — Claude handles the hard cases where
// Spotify tags are absent or too niche (phonk, hyperpop, plugg, sad rap, etc.)
const GENRE_RULES = [
  // Lo-Fi before Hip-Hop so "lofi hip hop" maps correctly
  { genre: 'Lo-Fi',      re: /lo.?fi|chillhop|jazz.?hop|study beats|\bjazz\b|bossa nova|lounge|easy listening/ },
  // Expanded Hip-Hop: plugg, phonk, sad rap, melodic rap, mumble, rage, jersey club
  { genre: 'Hip-Hop',    re: /hip.?hop|\brap\b|trap|drill|grime|afrobeats|afropop|cloud rap|plugg|phonk|sad rap|melodic rap|mumble|rage rap|jersey club|gangsta|crunk|bounce|hyphy|chopped/ },
  // R&B: alternative r&b, neo r&b, quiet storm, new jack swing, trap soul
  { genre: 'R&B',        re: /r&b|rnb|neo.?soul|\bsoul\b|\bfunk\b|quiet storm|urban contemporary|contemporary r|rhyth|trap soul|new jack|alternative r/ },
  // Electronic: synthwave, vaporwave, hyperpop, digicore, darksynth, future bass, uk garage, footwork
  { genre: 'Electronic', re: /electronic|edm|\bhouse\b|techno|trance|dubstep|drum.and.bass|\bdnb\b|synth.?pop|electro|ambient|synthwave|vaporwave|hyperpop|digicore|darksynth|future bass|uk garage|footwork|club|deconstructed|Jersey club|bass music|glitch/ },
  // Alt Rock: post-punk, britpop, math rock, noise rock, stoner rock, psych rock
  { genre: 'Alt Rock',   re: /\brock\b|alternative|grunge|\bpunk\b|\bmetal\b|\bemo\b|post.rock|hardcore|shoegaze|indie.rock|britpop|math rock|noise rock|stoner|psych rock|krautrock|new wave|post.punk/ },
  // Indie Pop: art pop, dark pop, hypnagogic, cottagecore, lo-fi pop, twee
  { genre: 'Indie Pop',  re: /indie.pop|bedroom.pop|dream.pop|indie|folk|singer.songwriter|art.pop|chillwave|acoustic|dark pop|hypnagogic|cottagecore|twee|freak folk|chamber pop/ },
  // Broad pop fallback — catches mainstream artists Spotify only tags as "pop"
  { genre: 'Indie Pop',  re: /\bpop\b/ },
]
...
// Vibe derivation from genre strings alone (no audio features required)
function deriveVibesFromGenres(spotifyGenres) {
  const set = new Set()
  const g = (spotifyGenres || []).join(' ').toLowerCase()
  if (/dance|dancehall|club/.test(g))                set.add('danceable')
  if (/psychedel/.test(g))                           set.add('psychedelic')
  if (/experimental|avant|noise/.test(g))            set.add('experimental')
  if (/lo.?fi|chillhop/.test(g))                     set.add('lo-fi')
  if (/punk|garage|diy/.test(g))                     set.add('raw / DIY')
  if (/vocal|acapella/.test(g))                      set.add('vocal-forward')
  if (/soul|r&b|rnb/.test(g))                        set.add('groovy')
  if (/ambient|atmospheric|drone/.test(g))            set.add('atmospheric')
  if (/dream|shoegaze|ethereal/.test(g))             set.add('dreamy')
  if (/synth|electronic|edm|house/.test(g))          set.add('synth-heavy')
  if (/folk|acoustic|singer.songwriter/.test(g))     set.add('guitar-driven')
  return [...set].filter(v => VIBES.includes(v)).slice(0, 4)
}
