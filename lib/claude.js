...
OUTPUT SCHEMA (follow exactly):
{
  ...
  "region": "<ISO 3166-1 alpha-2 country code for the artist's PRIMARY market, e.g. 'US', 'GB', 'KR', 'BR', 'NG', 'JP', 'ES', 'FR', 'CA', 'AU'>",
  "language": "<ISO 639-1 code for the language used in their music, e.g. 'en', 'ko', 'es', 'pt', 'fr', 'ja', 'yo'>",
  "sounds": [
    {
      "id": 1,
...
export async function generateArtistStrategy(artistData) {
  ...
  strategy.region   = typeof strategy.region   === 'string' && /^[A-Z]{2}$/.test(strategy.region)   ? strategy.region   : null
  strategy.language = typeof strategy.language === 'string' && /^[a-z]{2,3}$/.test(strategy.language) ? strategy.language : null
